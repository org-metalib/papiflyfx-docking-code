...
2. Add soft wrapping controlled by `wordWrap`.
...
6. Implement scrollbars fully from scratch (no `javafx.scene.control.ScrollBar`) — canvas-rendered via `ScrollbarPass` for visual consistency, pixel-perfect control, and alignment with the project's no-CSS philosophy.
...
1. Default: `wordWrap=false`, `horizontalScrollOffset=0.0`.
2. Transition `false → true` forces `horizontalScrollOffset=0.0`.
3. Transition `true → false` keeps `horizontalScrollOffset=0.0` (user can scroll manually).
4. `setHorizontalScrollOffset(...)` is a no-op in wrap mode except normalizing to `0.0`.

### 4.4 WrapMap — the soft-wrap layout model
...
final class WrapMap {
    ...
    // Used after single-line edits for performance (phase 5 optimization).
...
#### 4.5.3 Viewport.buildRenderLines (wrap-aware)
...
#### 4.5.4 Per-pass rendering adjustments
...
#### 4.5.6 Hit-testing in wrap mode
...
#### 4.5.7 Caret navigation in wrap mode
...
### 4.6 Disabling box selection in wrap mode
...
## 5. Feature 2: Scrollbars

### 5.1 Design choice: fully custom scrollbar built from scratch
...
### 5.2 Scrollbar visual specification

#### 5.2.1 Vertical scrollbar
...
The semi-transparent thumb over the track (which matches the editor background) produces a subtle, modern scrollbar appearance similar to VS Code. Theme changes take effect on the next redraw — no CSS reload, no node re-skinning.

### 5.4 New render pass: ScrollbarPass
...
### 5.5 Scrollbar interaction handling (raw mouse events)
...
#### 5.5.3 Hover state tracking and visual feedback
...
### 5.7 Scrollbar visibility and range rules

#### Vertical scrollbar

Visible when `maxVerticalOffset > 0`.

Range:
- `min=0`
- `max=maxVerticalOffset`
- `value=verticalScrollOffset`
- `visibleAmount=viewportHeight` (proportional thumb policy)
- Thumb size enforces minimum pixel size (>= `MIN_THUMB_SIZE`)
...
- `value=horizontalScrollOffset`
- `visibleAmount=viewportWidth`
- Thumb size enforces minimum pixel size (>= `MIN_THUMB_SIZE`)

Forced states: if `wordWrap=true`, horizontal bar is hidden/disabled and value reset to `0`.

#### Longest-line width computation

`contentPixelWidth = maxLineLengthChars * charWidth`

Phase-1 approach: full recompute on document changes for correctness. Optimization path: incremental line-length metrics cache once baseline is stable.

### 5.8 Horizontal scroll offset
...
### 5.9 Horizontal scroll in render passes
...
### 5.10 Scroll coordinator

Introduce `EditorScrollCoordinator` to centralize scroll state synchronization and avoid property feedback loops:
...
2. Normalize/clamp offsets when viewport size changes, document changes, wrap flag changes, or font metrics change.
3. Keep gutter vertical offset synchronized with viewport vertical offset.

Loop safety: guard booleans similar to existing `syncingScrollOffset` pattern in `EditorCaretCoordinator`.

### 5.11 Scroll wheel / trackpad enhancements
...
### 5.12 Search/GoToLine overlay margin adjustment
...
Existing v2 state payloads restore identically — the editor opens in unwrapped mode with no horizontal offset, which matches current behavior. Unknown/future versions still fallback safely to empty/default state.

### 6.4 State capture and restore
...
1. `render/ScrollbarPassTest`:
   - Thumb geometry math for edge values.
   - Min thumb size behavior.
2. `render/WrapMapTest`:
   - Logical line ↔ visual row mapping correctness.
   - Prefix-sum array integrity.
   - Rebuild on document/width/font changes.
3. `render/ViewportTest`:
   - Horizontal offset affects hit-testing and rendering positions.
   - Wrap mode row mapping correctness.
   - Vertical max changes with wrap.
   - Effective area calculation with scrollbar presence.
4. `state/EditorStateCodecTest`:
   - v3 encode/decode fields.
   - v2 migration defaults.
5. `theme/CodeEditorThemeMapperTest`:
   - Scrollbar color tokens mapped for both light/dark themes.

### 9.2 Integration tests (TestFX)

Add/update:

1. `api/CodeEditorIntegrationTest`:
   - Scrollbar visibility in wrap vs non-wrap modes.
   - Horizontal offset persistence.
   - Thumb drag updates scroll offset proportionally.
   - Track click jumps scroll position.
   - Verify the UI uses canvas-rendered scrollbars and no JavaFX `ScrollBar`.
2. `api/MouseGestureTest`:
   - `Shift+Alt+Drag` creates box selection only when wrap is off.
   - In wrap mode same gesture produces normal range selection.
   - Middle-drag box selection disabled in wrap mode.

### 9.3 Benchmark/regression checks
...
| Mixed multi-caret behavior with wrap toggle | Low | Deterministic wrap-mode rule set and explicit tests |
...
14. Scrollbar UI is canvas-rendered via `ScrollbarPass` and does not depend on `javafx.scene.control.ScrollBar`.
